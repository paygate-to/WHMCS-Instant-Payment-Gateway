...
$hrs_bitnovocom_gen_wallet = file_get_contents('https://api.highriskshop.com/control/wallet.php?address=' . $walletAddress .'&callback=' . urlencode($callback_URL));
	...
	$hrs_bitnovocom_wallet_decbody = json_decode($hrs_bitnovocom_gen_wallet, true);

 // Check if decoding was successful
    if ($hrs_bitnovocom_wallet_decbody && isset($hrs_bitnovocom_wallet_decbody['address_in'])) {
        // Store the address_in as a variable
        $hrs_bitnovocom_gen_addressIn = $hrs_bitnovocom_wallet_decbody['address_in'];
        $hrs_bitnovocom_gen_polygon_addressIn = $hrs_bitnovocom_wallet_decbody['polygon_address_in'];
		$hrs_bitnovocom_gen_callback = $hrs_bitnovocom_wallet_decbody['callback_url'];
		
		
		 // Update the invoice description to include address_in
            $invoiceDescription = "Payment reference number: $hrs_bitnovocom_gen_polygon_addressIn";
...
return "Error: Payment could not be processed, please try again (wallet address error)";
        ...
        $paymentUrl = 'https://pay.highriskshop.com/process-payment.php?address=' . $hrs_bitnovocom_gen_addressIn . '&amount=' . $hrs_bitnovocom_final_total . '&provider=bitnovo&email=' . urlencode($email) . '&currency=' . $hrs_bitnovocom_currency;
